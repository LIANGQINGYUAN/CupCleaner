...
def compute_sari(srcs, tgts, predictions):
    source_sentences = srcs#[ex.old_comment for ex in test_data]
    target_sentences = [[t] for t in tgts]#[[ex.new_comment] for ex in test_data]
    predicted_sentences = [' '.join(p) for p in predictions]
...
def compute_gleu(len_data, orig_file, ref_file, pred_file):
