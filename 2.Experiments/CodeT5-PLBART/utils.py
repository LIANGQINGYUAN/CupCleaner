...
def load_and_cache_gen_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    # print("test source", examples[0].source)
    # import sys
    # sys.exit()
...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'concode':
        ...
        if "_" in sub_task:
            subtask_tag = sub_task.split("_")[0] # such as 'ACL20'
            refine_tag = '_'+sub_task.split("_")[1] 
        else:
            subtask_tag = sub_task
            refine_tag = ''
        data_dir = '{}/{}/{}'.format(data_root, task, subtask_tag)
        train_fn = '{}/train{}.json'.format(data_dir,refine_tag)
        # using original dev set
        if 'odev' in sub_task:
            dev_fn = '{}/valid.json'.format(data_dir)
        else:
            dev_fn = '{}/valid{}.json'.format(data_dir,refine_tag)
        test_fn = '{}/test.json'.format(data_dir)

        # sample-based training
        if ',' in sub_task:
            files = sub_task.split(',')
            assert len(files) == 4
            subtask_tag = files[0].split('_')[0]
            data_dir = '{}/{}/{}'.format(data_root, task, subtask_tag)
            train_fn = '{}/{}'.format(data_dir, files[1])
            dev_fn = '{}/{}'.format(data_dir, files[2])
            test_fn = '{}/{}'.format(data_dir, files[3])
        ...
        data_dir = '{}/{}'.format(data_root, task)
        train_fn = '{}/data-defect-train.jsonl'.format(data_dir)
        dev_fn = '{}/data-defect-valid.jsonl'.format(data_dir)
        test_fn = '{}/data-defect-test.jsonl'.format(data_dir)
    elif task=='quality_detect':
        data_dir = '{}/{}/{}'.format(data_root, task, sub_task)
        # data_dir = '{}/{}'.format(data_root, task)
        train_fn = '{}/train.json'.format(data_dir)
        dev_fn = '{}/valid.json'.format(data_dir)
...
def read_examples(filename, data_num, task):
    read_example_dict = {
        ...
        'comment_update': read_comment_update_examples,
        'quality_detect': read_quality_detect_examples
    }
