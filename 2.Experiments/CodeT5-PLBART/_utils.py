...
def add_lang_by_task(target_str, task, sub_task):
    if task == 'summarize' or task == 'comment_update':
...
def read_comment_update_examples(filename, data_num):
    """Read examples from filename."""
    examples = []
    ...
    with open(filename) as f:
        for idx, line in enumerate(f):
            ...
            # original source
            s = x["old_comment"].strip()+ " <s> " + x["old_code"].strip()+ " <s> "+x["new_code"].strip().replace("\n"," ")
            # contains diff
            # span_diff_code = ' '.join(diff_utils.compute_minimal_comment_diffs(x['old_code'].split(), x['new_code'].split())[0])
            # s = x["old_comment"].replace("\n"," ").strip() + " <s> " + span_diff_code.strip()

            source = " ".join(s.split()).strip()
            
            old_comm = ' '.join(x["old_comment"].replace("\n"," ").strip().split())
            new_comm = ' '.join(x["new_comment"].replace("\n"," ").strip().split())

            # generate original
            t = new_comm
            # generate changes
            # span_minimal_diff_comment = ' '.join(diff_utils.compute_minimal_comment_diffs(old_comm.split(), new_comm.split())[0])
            # t = span_minimal_diff_comment.strip()
...
def read_quality_detect_examples(filename, data_num):
    """Read examples from filename."""
    examples = []
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            line = line.strip()
            x = json.loads(line)
            code = x["old_comment"].strip()+" "+' '.join(x["span_minimal_diff_comment"]).strip()+" "+x["old_code"].strip()\
                +" "+' '.join(x["span_minimal_diff_code"]).strip().replace("\n"," ")
            examples.append(
                Example(
                    idx=idx,
                    source=code,
