...
def get_args_by_task_model(task, sub_task, model_tag):
    if task == 'translate':
        ...
        epoch = 30
        patience = 3
    elif task == 'comment_update':
        # aaai
        # Read 16494 examples, avg src len: 107, avg trg len: 10, max src len: 4162, max trg len: 111
        # [TOKENIZE] avg src len: 302, avg trg len: 14, max src len: 16425, max trg len: 193
        # ase
        # Read 85657 examples, avg src len: 84, avg trg len: 10, max src len: 1779, max trg len: 56
        # [TOKENIZE] avg src len: 238, avg trg len: 15, max src len: 2931, max trg len: 100
        # src_len = 350
        # trg_len = 150
        epoch = 20
        patience = 3
        ...
        if 'ASE' in sub_task:
            src_len = 512
            trg_len = 100
        elif 'ACL' in sub_task:
            src_len = 350
            trg_len = 100
        elif 'AAAI' in sub_task:
            if 'plbart' in model_tag:
                src_len = 350
                trg_len = 100
            else:
                src_len = 512
                trg_len = 100
    elif task == 'defect' or 'quality_detect':
...
def run_one_exp(args):
    ...
    cmd_str = get_cmd(task=args.task, sub_task=args.sub_task, model_tag=args.model_tag, gpu=args.gpu,
                      data_num=args.data_num, bs=args.bs, lr=lr, source_length=src_len, target_length=trg_len,
...
def get_sub_tasks(task):
    if task == 'summarize':
        ...
        sub_tasks = ['none']
    elif task in ['quality_detect']:
        sub_tasks = ['pn43','pn42']
    elif task == 'comment_update':
        sub_tasks = ['ACL20','AAAI21','ASE20','ASE20_refine5','ASE20_refine10','ASE20_refine15','ASE20_refine20']
    return sub_tasks
