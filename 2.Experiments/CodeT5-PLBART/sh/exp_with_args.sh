...
elif [[ $MODEL_TAG == plbart_base ]]; then
  MODEL_TYPE=plbart
...
elif [[ ${TASK} == 'defect' || ${TASK} == 'quality_detect' ]] && [[ ${MODEL_TYPE} == 'roberta' ||  ${MODEL_TYPE} == 'bart' ]]; then
  RUN_FN=${WORKDIR}/run_defect.py
elif [[ ${TASK} == 'dynamic_gen' ]]; then
  RUN_FN=${WORKDIR}/run_gen_d.py
else
